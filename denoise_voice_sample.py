...
CLEAN_REF_OUT = ASSETS_DIR / "matatu.wav"
...
def denoise_and_enhance(audio_path: str, output_path: str, start_sec: float = 2.0, duration_sec: float = 10.0):
    ...
    # RMS-normalize to -16 dBFS (about 0.158 linear)
    target_rms = 10 ** (-16 / 20)   # ~0.158
    current_rms = np.sqrt(np.mean(filtered_audio ** 2))
    if current_rms > 0:
        normalized_audio = filtered_audio * (target_rms / current_rms)
    else:
